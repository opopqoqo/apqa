...
void decomposeOrtho(vector3f u,vector3f v,
	...
	float normOfU0=projectOrtho(&u, &v);
	v3fSet(u0, v);
...
vector3f v3fYPRrad(const vector3f* u){
	
	vector2f u0 = { u->x, u->z };
	float l = v2fLength(&u0);
	v2fMul(&u0, 1.0f/l);
	float alpha = v2fAngleRadRelativeToXAxis(&u0);
	float beta = atanf(u->y/l);
	
	vector3f ypr = { beta, alpha, 0};
 ...
 */
vector3f projectPointOnLine(vector3f* P,vector3f* A,vector3f* u){
    
    vector3f v=v3fMake(A,P);
    float x=projectOrtho(&v, u);
 ...
 */
float projectOrtho(vector3f* u,vector3f* v){
...
void v3fAdd(vector3f* u,vector3f* v){
	u->x+=v->x;
	u->y+=v->y;
	u->z+=v->z;
}

void v3fSub(vector3f* u,vector3f* v){
	u->x-=v->x;
	u->y-=v->y;
	u->z-=v->z;
}
...
char v3fEquals(vector3f u,vector3f v){
	if(u.x!=v.x)return 0;
	if(u.y!=v.y)return 0;
	if(u.z!=v.z)return 0;
	return 1;
...
char v3fIsNaN(const vector3f* u){
	if(isnan(u->x))return 1;
	if(isnan(u->y))return 1;
	if(isnan(u->z))return 1;
	return  0;
}
...
char v3fIsNormalizable(vector3f u){
	float l=v3fLength(&u);
	float a=u.x/l;
	if(isnan(a))return 0;
	a=u.y/l;
...
float v3fXZDistanceSquared(vector3f* a,vector3f* b){
	float x=b->x-a->x;
	float z=b->z-a->z;
	return x*x+z*z;
}

char v3fIsZero(vector3f* u){
	if(u->x!=0)return 0;
	if(u->y!=0)return 0;
	if(u->z!=0)return 0;
	return 1;
